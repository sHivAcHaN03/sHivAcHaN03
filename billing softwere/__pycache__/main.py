...
img18=PhotoImage(file="create.png")
...
##########Create  in bill page ###############################################
            ...
            def CREATE():
                ...
                con1=mysql.connector.connect(host="localhost",user="root",password="kitty03",database="siva")
                res1=con1.cursor()
                sql1="insert into customer(cust_name,cust_ph,cust_add,dis_price,s_rep,bill_type) values(%s,%s,%s,%s,%s,%s);"
                user1=(cname,cph,cadd,disp,srep,billty)
...
#############search in bill page ###############################################
            ...
            mytree["columns"]=('S.No','Product Name','Quantity','Rate','Dis Amt','Dis%','stockIn date','Expiry date ','GST')
            mytree.column('#0',width=20,stretch=NO)
...
#stretch=NO means it will remove the 0th column
            mytree.column("#1",width=60,anchor=CENTER)
            mytree.column("#2",width=150,anchor=CENTER)
            mytree.column("#3",width=80,anchor=CENTER)
            mytree.column("#4",width=60,anchor=CENTER)
            mytree.column("#5",width=60,anchor=CENTER)
            mytree.column("#6",width=50,anchor=CENTER)
            mytree.column("#7",width=180,anchor=CENTER)
            mytree.column("#8",width=180,anchor=CENTER)
            mytree.column("#9",width=60,anchor=CENTER)           
            mytree.heading("#0",text="")
            mytree.heading("#1",text="Sno")
            mytree.heading("#2",text="Product Name")
            mytree.heading("#3",text="Quantity")
...
##########Delete in bill page ###############################################
            def total():
                ...
                print("your amount is",amount)
                        ##bill menu area       
            lbl3=LabelFrame(win4,font=("tiems",16,"bold"),bg="#4f4557",fg="white",relief=GROOVE,bd=5)
            lbl3.place(x=740,y=610,width=600,height=100)
            BTN1=Button(win4,text="TOTAL",command=total,font=("times",15,"bold"),bg="yellow",activebackground="#4f4557",fg="black",activeforeground="white")
            BTN1.place(x=760,y=640)
            BTN2=Button(win4,text="BILL",command=None,font=("times",15,"bold"),bg="yellow",activebackground="#4f4557",fg="black",activeforeground="white")
            BTN2.place(x=880,y=640)
            BTN3=Button(win4,text="PRINT",command=None,font=("times",15,"bold"),bg="yellow",activebackground="#4f4557",fg="black",activeforeground="white")
            BTN3.place(x=990,y=640)
            BTN3=Button(win4,text="DELETE",command=None,font=("times",15,"bold"),bg="yellow",activebackground="#4f4557",fg="black",activeforeground="white")
            BTN3.place(x=1100,y=640)
            BTN4=Button(win4,text="CLEAR",command=None,font=("times",15,"bold"),bg="yellow",activebackground="#4f4557",fg="black",activeforeground="white")
            BTN4.place(x=1230,y=640)        
            ...
            #BTN1.place(x=450,y=650)
            #BTN2=Button(win4,image=img19,bd=0,command=None,bg="#4f4557",fg="white",activebackground="#4f4557")
            #BTN2.place(x=600,y=650)
            LBL7=Label(win4,text="CREATE",font=("times",12,"bold"),bg="#4f4557",fg="white")
            LBL7.place(x=445,y=695)
            LBL8=Label(win4,text="UPDATE",font=("times",12,"bold"),bg="#4f4557",fg="white")
            LBL8.place(x=595,y=695)
            #######bill area         
            w4F1=Frame(win4,width=440,height=400,relief=GROOVE)
            w4F1.place(x=910,y=145)
            w4F1.configure(bg="white")
            bill_title=Label(w4F1,text="Bill Area",font=("Arial Black",15),bd=4,relief=GROOVE,width=32,bg="#4f4557",fg="white").place(x=0,y=0)
            last_bill=Label(win4,text="Last Billl",font=("times",11,"bold"),bg="#4f4557",fg="white")
            last_bill.place(x=910,y=578)
            lastb_entry=Entry(win4,font=("times",9,"bold"),bg="#4f4557",fg="white",bd=0)
            lastb_entry.place(x=990,y=578)
            totamt=Label(win4,text="Last Billl",font=("times",11,"bold"),bg="#4f4557",fg="white")
            totamt.place(x=1100,y=578)
            totamt_entry=Entry(win4,font=("times",9,"bold"),bg="#4f4557",fg="white",bd=0)
            totamt_entry.place(x=1170,y=578)
                ###scrollbar in bill            
            scrol_y=Scrollbar(w4F1,width=15,orient=VERTICAL)
            txtarea=Text(w4F1,width=53,height=22,yscrollcommand=scrol_y.set)
            scrol_y.pack(side=RIGHT,fill=Y)
            scrol_y.config(command=txtarea.yview)
            txtarea.pack(padx=0,pady=37,fill=BOTH,expand=0)
            txtarea.insert(END,"\t\tWELCOME TO CHAN BILLING\n\t\tPhone-No.833798693")
            txtarea.insert(END,f"\n\nBill no. : {bill_no.get()}\t\t{bill_no.get()}")
           # txtarea.insert(END,f"\t\t  {w4E2.get()}")
            txtarea.insert(END,f"\nCustomer Name : {c_name.get()}")
            txtarea.insert(END,f"\nPhone No. : {phone.get()}")
            txtarea.insert(END,"\n=====================================================")
            txtarea.insert(END,"\nSno\tProduct\t\tQty\tRate\tPrice")
            txtarea.insert(END,"\n=====================================================\n")
            be1=Entry(win4,font=("times",8,"bold"),bg="red",fg="white",width=1)
            be1.place(x=1180,y=220)
            global l
            l=[]
            def displaySelectedItem(a):
                ...
                c=Sno2
                slv=[]
                slv.append(mytree.selection())
                print(slv)
                
                print(mytree.selection())
                r2=simpledialog.askstring("Note","Enter Your Qty")
                selectedItem=mytree.selection()[0]
                txtarea.insert(END,Sno2)
                txtarea.insert(END,"\t")
                d=int(r2)*int(mytree.item(selectedItem)['values'][3])
                print(d)
                txtarea.insert(END,mytree.item(selectedItem)['values'][1])
                txtarea.insert(END,"\t\t")
                txtarea.insert(END,r2)
                txtarea.insert(END,"\t")
                txtarea.insert(END,mytree.item(selectedItem)['values'][3])
                txtarea.insert(END,"\t")
                txtarea.insert(END,d)
                l.append(d)
                for n in range(0,11):
                    print(mytree.item(selectedItem)['values'][n])
                txtarea.insert(END,"\n")
    ...
    w2b0.place(x=330,y=530)
###page 1 button
f2b0=Button(f2,text="Sign Up",font=("times",12,"bold"),activebackground="skyblue",bg="#0a457f",fg="yellow",width=7,height=1,command=sign)
f2b0.place(x=20,y=580)

f2b0=Button(f2,image=img7,bg="#bc88f9",command=sign,activebackground="#bc88f9",fg="black",bd=0,width=0,height=0)
f2b0.place(x=290,y=580)
